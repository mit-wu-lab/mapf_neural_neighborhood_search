...
PSolver::PSolver(PProblem* _P) : P(_P) {
  std::random_device seed_gen;
  MT = new std::mt19937(seed_gen());
  init();
}
PSolver::PSolver(PProblem* _P, std::mt19937* _MT) : P(_P), MT(_MT) {
  init();
}
...
void PSolver::solveStart() {
  startT = std::chrono::system_clock::now();
}

void PSolver::solveEnd() {
  ...
  for (int i = 0; i < A.size(); ++i) {
    for (int j = i + 1; j < A.size(); ++j) {
      if (paths[i].size() != paths[j].size()) {
        std::cout << "error@PSolver, path size is different" << std::endl;
        std::exit(1);
      }
      for (int t = 0; t < paths[i].size(); ++t) {
        if (t > 0) {
          ...
          if (!inArray(paths[i][t], cands)) {
            std::cout << "error@PSolver, paths is not connected at t=" << t << ", "
                      << "agent " << i
        ...
        if (paths[i][t] == paths[j][t]) {
          std::cout << "error@PSolver, vertex conflict at t=" << t << " between "
                    << i << " and " << j << std::endl;
          std::exit(1);
        }
        if (t > 0 && paths[i][t] == paths[j][t-1] && paths[i][t-1] == paths[j][t]) {
          std::cout << "error@PSolver, swap conflict at t=" << t << " between "
                    << i << " and " << j << std::endl;
          std::exit(1);
        }
      }
...
int PSolver::getMaxLengthPaths(Paths& paths) {
...
void PSolver::formalizePath(Paths& paths) {
...
int PSolver::pathDist(Node* s, Node* g) {
  // same place?
  if (s == g) return 0;
  ...
  for (auto v : path) {
    ...
    if ((index != g_index) && (d == 0)) {
      ...
      std::cout << "error@PSolver::pathDist, "
                << s->getId() << " -> " << g->getId() << ", "
                << s->getPos() << " -> " << g->getPos() << ", "
...
int PSolver::pathDist(Node* s, Node* g, Nodes &prohibited) {
  // same place?
  if (s == g) return 0;
...
std::string PSolver::getKey(int t, Node* v) {
  std::string key = "";
  key += std::to_string(t);
  key += "-";
...
std::string PSolver::getKey(AN* n) {
  return getKey(n->g, n->v);
}

std::string PSolver::logStr() {
  std::string str;
